...
import { flipkartCollectionFee, flipkartFixedFee, flipkartShippingFee, getIndiaMarketplaceMetadata, isFlipkartZeroCommissionEligible, type FlipkartFulfillment, type FlipkartPaymentMode, type FlipkartZone } from '@/lib/india-marketplaces';

type Props = { initialPlatform?: Platform };
type State = CalculatorInput & { platform: Platform; country: CountryCode; currency: CurrencyCode; productName: string; category: Category; language: LanguageCode; indiaFulfillment: FlipkartFulfillment; indiaPaymentMode: FlipkartPaymentMode; indiaZone: FlipkartZone; packedWeightKg: number };
...
function initial(platform: Platform = 'Amazon', country: CountryCode = 'US', language: LanguageCode = 'en'): State {
  ...
  return { productName: 'Sample Product', platform: valid, country: c.code, currency: c.currency, language, category: 'Other', sellingPrice: 49.99, quantity: 1, productCost: 15, marketplaceFixedFee: cost(false, 0, 'fixed'), collectionFee: cost(false, 0, 'fixed'), platformFee: cost(true, fee), paymentFee: cost(true, 2.9), shipping: cost(true, 5, 'fixed'), packaging: cost(true, 0.5, 'fixed'), advertising: cost(true, 8, 'fixed'), affiliate: cost(false, 0), discount: cost(false, 0), returns: cost(true, 3), rto: cost(false, 0), tax: cost(false, 0), other: cost(false, 0), indiaFulfillment: 'NFBF', indiaPaymentMode: 'Prepaid', indiaZone: 'National', packedWeightKg: 0.5 };
}
...
export default function Calculator({ initialPlatform = 'Amazon' }: Props) {
  ...
  const applyIndiaMarketplaceModel = (prev: State, platform: Platform, overrides: Partial<Pick<State, 'sellingPrice' | 'category' | 'indiaFulfillment' | 'indiaPaymentMode' | 'indiaZone' | 'packedWeightKg'>> = {}) => {
    const next = { ...prev, ...overrides, platform };
    if (next.country !== 'IN' || (platform !== 'Flipkart' && platform !== 'Meesho')) {
      const fee = getPlatformConfig(platform, next.country)?.fee.value ?? 0;
      return { ...next, platformFee: cost(true, fee), marketplaceFixedFee: cost(false, 0, 'fixed'), collectionFee: cost(false, 0, 'fixed') };
    }
    if (platform === 'Meesho') {
      return { ...next, platformFee: cost(true, 0), marketplaceFixedFee: cost(false, 0, 'fixed'), collectionFee: cost(false, 0, 'fixed') };
    }
    const eligibility = isFlipkartZeroCommissionEligible(next.sellingPrice, next.category);
    const commission = eligibility.eligible ? 0 : (getPlatformConfig('Flipkart', 'IN')?.fee.value ?? 0);
    const fixed = flipkartFixedFee(next.sellingPrice, next.indiaFulfillment);
    const collection = flipkartCollectionFee(next.sellingPrice);
    const shipping = flipkartShippingFee(next.packedWeightKg, next.indiaZone);
    return {
      ...next,
      platformFee: cost(true, commission),
      marketplaceFixedFee: fixed === 0 ? cost(false, 0, 'fixed') : cost(true, fixed, 'fixed'),
      collectionFee: collection === 0 ? cost(false, 0, 'fixed') : cost(true, collection, 'fixed'),
      shipping: Number.isFinite(shipping) ? cost(true, shipping, 'fixed') : next.shipping
    };
  };
  ...
  const countryChange = (code: CountryCode) => {
    ...
    setS((prev) => applyIndiaMarketplaceModel({ ...prev, country: code, currency: c.currency }, platform));
  };

  const platformChange = (platform: Platform) => {
    setS((prev) => applyIndiaMarketplaceModel(prev, platform));
  };
  ...
  const isIndiaMarketplace = s.country === 'IN' && (s.platform === 'Flipkart' || s.platform === 'Meesho');
  const indiaMeta = isIndiaMarketplace ? getIndiaMarketplaceMetadata(s.platform as 'Flipkart' | 'Meesho') : null;

  const updatePrice = (value: number) => setS((prev) => applyIndiaMarketplaceModel(prev, prev.platform, { sellingPrice: value }));
  const updateCategory = (value: Category) => setS((prev) => applyIndiaMarketplaceModel(prev, prev.platform, { category: value }));
  const updateIndiaHelper = (patch: Partial<Pick<State, 'indiaFulfillment' | 'indiaPaymentMode' | 'indiaZone' | 'packedWeightKg'>>) => setS((prev) => applyIndiaMarketplaceModel(prev, prev.platform, patch));
  ...
  const comparisons = useMemo(() => platformConfigs.filter((p) => p.countries.includes(s.country)).map((p) => {
    const modeled = applyIndiaMarketplaceModel(s, p.platform);
    const next = modeled as CalculatorInput;
    return { ...p, modeled, profit: calculateNetProfit(next), margin: calculateProfitMargin(next) };
  }).sort((a, b) => b.profit - a.profit), [s]);

  const countryComparisons = useMemo(() => countries.map((c) => {
    const p = getPlatformsForCountry(c.code)[0];
    const next = applyIndiaMarketplaceModel({ ...s, country: c.code, currency: c.currency }, p.platform) as CalculatorInput;
  ...
  const costRows: Array<[keyof CalculatorInput, string, 'percent' | 'fixed']> = [
    ['platformFee', tr('fees'), 'percent'], ['marketplaceFixedFee', 'Marketplace fixed fee', 'fixed'], ['collectionFee', 'Collection fee', 'fixed'], ['paymentFee', tr('payment'), 'percent'], ['shipping', tr('shipping'), 'fixed'], ['packaging', tr('packaging'), 'fixed'], ['advertising', tr('advertising'), 'fixed'], ['affiliate', tr('affiliate'), 'percent'], ['discount', tr('discount'), 'percent'], ['returns', tr('returns'), 'percent'], ['rto', tr('rto'), 'percent'], ['tax', tr('tax'), 'percent'], ['other', tr('other'), 'fixed']
  ];
    ...
    {tab === 'calculator' && <div className="calculator">
      <div className="card panel">
        ...
        <div className="form-grid">
          ...
          <div className="field"><label htmlFor="category">{tr('category')}</label><select id="category" value={s.category} onChange={(e) => updateCategory(e.target.value as Category)}>{categories.map((c) => <option key={c}>{c}</option>)}</select></div>
          <div className="field"><label htmlFor="price">{tr('sellingPrice')}</label><input id="price" type="number" min="0" step="0.01" value={s.sellingPrice} onChange={(e) => updatePrice(clamp(Number(e.target.value)))} /></div>
        ...
        {isIndiaMarketplace && <div className="india-market-card">
          <div className="section-head"><div><h3 style={{ marginTop: 0 }}>{s.platform === 'Flipkart' ? 'Flipkart India rate-card helper' : 'Meesho India marketplace helper'}</h3><p className="section-copy" style={{ marginBottom: 0 }}>{indiaMeta?.description}</p></div><span className="trust-chip">Official source • Sep 2026 check</span></div>
          {s.platform === 'Flipkart' ? <div className="form-grid" style={{ marginTop: 14 }}>
            <div className="field"><label htmlFor="fk-fulfillment">Fulfilment</label><select id="fk-fulfillment" value={s.indiaFulfillment} onChange={(e) => updateIndiaHelper({ indiaFulfillment: e.target.value as FlipkartFulfillment })}><option value="NFBF">NFBF — Non Fulfilment by Flipkart</option><option value="FBF">FBF — Fulfilment by Flipkart</option></select></div>
            <div className="field"><label htmlFor="fk-payment">Customer payment</label><select id="fk-payment" value={s.indiaPaymentMode} onChange={(e) => updateIndiaHelper({ indiaPaymentMode: e.target.value as FlipkartPaymentMode })}><option value="Prepaid">Prepaid</option><option value="COD">COD</option></select></div>
            <div className="field"><label htmlFor="fk-zone">Shipping zone</label><select id="fk-zone" value={s.indiaZone} onChange={(e) => updateIndiaHelper({ indiaZone: e.target.value as FlipkartZone })}><option value="Local">Local</option><option value="Zonal">Zonal</option><option value="National">National</option></select></div>
            <div className="field"><label htmlFor="fk-weight">Packed weight (kg)</label><input id="fk-weight" type="number" min="0.1" step="0.1" value={s.packedWeightKg} onChange={(e) => updateIndiaHelper({ packedWeightKg: clamp(Number(e.target.value), 0.1, 5) })} /></div>
          </div> : <div className="notice" style={{ marginTop: 14 }}><strong>0% commission:</strong> Meesho states that suppliers pay zero commission. Shipping and other seller-specific deductions are not assumed here; enter your actual/current values below.</div>}
          <div className="notice" style={{ marginTop: 12 }}><strong>What the helper is doing:</strong> {s.platform === 'Flipkart' ? 'It models the public standard fixed, collection and weight/zone shipping components, plus the published zero-commission eligibility for qualifying sub-₹1,000 products and fashion. Category-specific commission outside those cases remains an editable/user-verifiable assumption.' : 'It sets marketplace commission to 0% and leaves shipping, returns/RTO, ads and other seller-specific costs to your inputs.'}</div>
          {indiaMeta?.source && <p className="input-note"><a href={indiaMeta.source} target="_blank" rel="noreferrer">Open official marketplace source →</a></p>}
        </div>}
      ...
      <div className="results">
        <div className="card panel">
          ...
          <div className="breakdown">{Object.entries({ Revenue: result.line.revenue, 'Product cost': -result.line.product, 'Marketplace fee': -result.line.platform, 'Marketplace fixed fee': -result.line.marketplaceFixed, 'Collection fee': -result.line.collection, 'Payment fee': -result.line.payment, Shipping: -result.line.shipping, Packaging: -result.line.packaging, Advertising: -result.line.advertising, Affiliate: -result.line.affiliate, Discount: -result.line.discount, Returns: -result.line.returns, RTO: -result.line.rto, Tax: -result.line.tax, Other: -result.line.other }).map(([label, value]) => <div className="breakdown-row" key={label}><span>{label}</span><span className={value < 0 ? 'negative' : ''}>{money(value, s.currency)}</span></div>)}</div>
          <details className="details"><summary>{tr('assumptions')}</summary><div className="assumption-grid"><span>{tr('fees')}</span><strong>{s.platformFee.mode === 'percent' ? `${s.platformFee.value}%` : money(s.platformFee.value, s.currency)}</strong><span>Source status</span><strong>{feeMeta?.status ?? 'user-defined'}</strong><span>Last verified</span><strong>{feeMeta?.lastVerified ?? 'Not verified'}</strong><span>{tr('category')}</span><strong>{s.category}</strong>{indiaMeta && <><span>India model</span><strong>{s.platform === 'Flipkart' ? `${s.indiaFulfillment} • ${s.indiaZone} • ${s.packedWeightKg}kg` : '0% commission model'}</strong></>}</div></details>
